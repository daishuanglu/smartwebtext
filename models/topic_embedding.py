...
class GlobalTopicAsEmbedding(ptl.LightningModule, ABC):
    def __init__(self, config, vae_config=VaeConfig()):
        super().__init__()
        self.config = config
        self.vocab = {line.strip(): i for i, line in enumerate(open(config['dict_path'], 'r'))}
        self.vocab_size = len(self.vocab) + 1
        self.vae = ProdLDA(asdict(vae_config), self.vocab_size).to(train_utils.device)

    def doc_tok_ids_count(self, batch):
        tok_counts = []
        for line in batch[self.config['text_col']]:
            if not line.strip():
                tok_count = torch.tensor([0] * self.vocab_size)
            else:
                tok_ids = torch.tensor([
                    self.vocab.get(word, self.vocab_size - 1) for word in line.split()])
                tok_ids_one_hot = F.one_hot(tok_ids, self.vocab_size)
                tok_count = torch.sum(tok_ids_one_hot, dim=0, keepdim=True).float()
            tok_counts.append(tok_count)
        batch_tok_counts = torch.cat(tok_counts, dim=0).to(train_utils.device)
        return batch_tok_counts
