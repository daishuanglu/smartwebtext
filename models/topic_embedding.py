    ...
    def doc_tok_ids_count(self, batch):
        ...
        for line in batch[self.config['text_col']]:
            tok_ids = torch.tensor([
                self.vocab.get(word, self.vocab_size) for word in line.split()])
            tok_ids_one_hot = F.one_hot(tok_ids.to(train_utils.device), self.vocab_size)
            tok_count = torch.sum(tok_ids_one_hot, dim=1, keepdim=True).float()
            tok_counts.append(tok_count)
        tok_counts = torch.cat(tok_counts, dim=1).to(train_utils.device)
        return tok_counts
