"""
The data_utils is a copy from my own github repo for a simple showcase demo purpose only.
Not a scalable solution.
https://github.com/daishuanglu/smartwebtext/blob/shud_wip/utils/data_utils.py
"""
...
def get_context_csv_data_loader(data_path, features_desc, batch_size=2048,
    ...
    if limit is None:
        nb_samples = sum(1 for _ in tqdm(open(data_path, 'r'))) - 1
        ...
        with open(limit_data_path, 'w') as fw:
            with open(data_path, 'r') as fr:
                for _ in range(limit + 1):
                    fw.write(next(fr))
        data_path = limit_data_path
        print('limit debugging {:d} samples'.format(limit))
